...
with st.sidebar:
    st.subheader("👵 Nonna")
    # Simple reactive 'mood' via session state; drop assets/nonna_{mood}.png if you have them
    st.session_state.setdefault("nonna_mood", "hello")
    mood = st.session_state["nonna_mood"]
    if os.path.exists(f"assets/nonna_{mood}.png"):
        st.image(f"assets/nonna_{mood}.png", use_container_width=True)
    else:
        st.markdown(f"""
        <div class="nonna">
          <div style="font-size:2rem">👵</div>
          <div>
            <div><strong>Nonna says:</strong></div>
            <div class="muted soft">“Tell me what you have, caro — we'll make something delicious.”</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    menu_path = st.text_input("…or menu path", "data/authentic_menu_priced_2025.csv")

    inv_upload = st.file_uploader("Inventory CSV", type=["csv"], help="Needs an 'ingredient' column (synonyms OK).")
    inv_path = st.text_input("…or inventory path", "data/authentic_inventory.csv")
    ...
    st.caption("Tip: Drop cute Nonna PNGs into ./assets as nonna_happy.png, nonna_alert.png, etc.")
...
if run:
    ...
    with tab4:
        ...
        <div class="metric-row">
          ...
          <span class="pill">⚠️ Use Soon: <b>{use_oon}</b></span>
          <span class="pill">❌ 86: <b>{total - ready - use_oon}</b></span>
        </div>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
